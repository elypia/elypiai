...
public class PathOfExileTest {
    ...
    public void parsePartialStashTabs() throws IOException {
        server.enqueue(new MockResponse().setBody(TestUtils.read("public-stash-tabs_single")));
    ...
    public void parseMultipleLeagues() throws IOException {
        server.enqueue(new MockResponse().setBody(TestUtils.read("leagues_default")));
    ...
    public void getSingleRule() throws IOException {
        server.enqueue(new MockResponse().setBody(TestUtils.read("rule_hardcore")));
    ...
    public void getMultipleRules() throws IOException {
        server.enqueue(new MockResponse().setBody(TestUtils.read("rules")));
    ...
    public void parseLadderEntry() throws IOException {
        server.enqueue(new MockResponse().setBody(TestUtils.read("ladders_standard")));
    ...
    public void parseLadderEntryWithGuild() throws IOException {
        server.enqueue(new MockResponse().setBody(TestUtils.read("ladders_standard-lab-norm-pc")));
    ...
    public void parsePvpMatches() throws IOException {
        server.enqueue(new MockResponse().setBody(TestUtils.read("pvp-matches_EUPvPSeason1")));
