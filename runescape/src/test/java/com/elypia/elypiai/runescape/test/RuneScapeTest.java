...
public class RuneScapeTest {
	...
	public void parseUser() throws IOException {
		server.enqueue(new MockResponse().setBody(TestUtils.read("profile_sethii")));
	...
	public void parseUserLeaderboards() throws IOException {
		server.enqueue(new MockResponse().setBody(TestUtils.read("profile_sethii")));
		server.enqueue(new MockResponse().setBody(TestUtils.read("profile_los")));
	...
	public void userDoesntExist() throws IOException {
		server.enqueue(new MockResponse().setBody(TestUtils.read("profile_no-profile")));
	...
	public void parseQuest() throws IOException {
		server.enqueue(new MockResponse().setBody(TestUtils.read("quests_sethii")));
	...
	public void parseNoQuests() throws IOException {
		server.enqueue(new MockResponse().setBody(TestUtils.read("quests_private")));
